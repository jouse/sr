...
class Model:
    ...
    def _add_lstm(self, input_, lstm_hidden_size, proj, layer_num):
        with tf.variable_scope( "lstm_layer_" + layer_num ):
            lstm_cell = [ LSTMCell( num_units=lstm_hidden_size, num_proj=proj, activation=tf.nn.tanh ) for i in range (1)]
            lstm = MultiRNNCell(lstm_cell)

            outputs, _ = tf.nn.dynamic_rnn( cell=lstm, inputs=input_, dtype=tf.float32,
    ...
    def build(self, reuse):
        ...
        with tf.variable_scope( self.name ) as vs:
            ...
            with tf.variable_scope( 'RNN' ):
                ...
                #layer_2 = self._add_2dfc_layer(layer_2, self.neural_number, self.neural_number, tf.nn.sigmoid, "3")
                #self.output_layer = self._add_2dfc_layer(layer_2, self.neural_number, self.output_dimension, tf.nn.softmax, "4")
    ...
    def train(self, read_ckpt=None):
        ...
        #voice_path = "\\data\\AI level2\\dataset\\subset\\*\\*"
        voice_path = "C:\\Project\\Python\\AISound\\dataset\\speech\\*\\*"
        noise_path = "C:\\Project\\Python\\AISound\\dataset\\noise\\*"
        dev_voice_path = "C:\\Project\\Python\\AISound\\dataset\\speech\\*\\*"
        dev_noise_path = "C:\\Project\\Python\\AISound\\dataset\\noise\\*"

        get_data_list = [tag for tag in iglob( voice_path )]

        data_list, dev_data_list = train_test_split(get_data_list, test_size=0.1, train_size=0.9,random_state=0)
        #print("data_list")
        ...
        with tf.Session() as sess:
            ...
            writer = tf.summary.FileWriter(
                self.tb_dir + '\\train', sess.graph, max_queue=10 )
            validation_writer = tf.summary.FileWriter( self.tb_dir + '\\validation', sess.graph )
            merge_op = tf.summary.merge_all()

            ## read ckpt
            if read_ckpt is not None:
                #model_path = "\\data\\AI level2\\model\\_verification\\" + read_ckpt + "\\"
                model_path = '.\\model\\_verification\\' + read_ckpt
    ...
    def test(self):
        ...
        voice_path = "C:\\Project\\Python\\AISound\\dataset\\test\\*\\*"
        noise_path = "C:\\Project\\Python\\AISound\\dataset\\noise\\*"
        train_path = "C:\\Project\\Python\\AISound\\dataset\\speech\\*\\*"

        get_test_list = [tag for tag in iglob( voice_path )]
        get_data_list = [tag for tag in iglob(train_path)]

        noise_list = [tag for tag in iglob( noise_path )]
        noise_list = np.random.choice( noise_list, len( get_test_list ) )

        snr_list = ['10dB']

        print("data_list")
        for data in get_test_list:
            print(data)
        ...
        with tf.Session() as sess:
            ...
            model_path = '.\\model\\_verification\\1213_2'
