...
class Model:
    ...
    def __init__(self, log_path, saver_path, date, gpu_num, note):
        ...
        self.filter_width = 3
        self.neural_number = (self.time_step-(self.filter_width-1))*self.output_channel
        self.output_dimension = 2
        self.init_learning_rate = 1e-4
        self.audio_batch = 10
        self.thread_num = 2
        self.batch_size = 10
        self.shuffle_data_time = 5
    ...
    def _add_conv2d_layer(self, input, layer_num, filter_h, filter_w, input_c, output_c, dilation, activate_function):
        with tf.name_scope("conv_layer_" + str(layer_num)):
            ...
            conv = tf.nn.conv2d( input, weights, strides=(1, 1, 1, 1),
                                 padding='VALID', dilations=dilation )
        ...
        with tf.variable_scope( self.name ) as vs:
            ...
            with tf.variable_scope( 'Intputs' ):
                self.x_noisy = tf.placeholder(
                    tf.float32, shape=[None, self.input_dimension, self.time_step, self.input_channel], name='x' )
            ...
            with tf.variable_scope( 'CNN' ):
                ...
                layer_1 = self._add_conv2d_layer(self.x_noisy, layer_num='1', filter_h=257, filter_w=self.filter_width,
                                                 input_c=self.input_channel, output_c=self.output_channel,
                                                 dilation=[1, 1, 1, 1], activate_function=tf.nn.relu)

                flatten_layer1 = tf.layers.Flatten()(layer_1)

                layer_2 = self._add_2dfc_layer(flatten_layer1, self.neural_number, self.neural_number, tf.nn.sigmoid, "2")
                self.output_layer = self._add_2dfc_layer(layer_2, self.neural_number, self.output_dimension, tf.nn.softmax, "3")
    ...
    def test(self):
        ...
        with tf.Session() as sess:
            ...
            model_path = './model/_verification/0708'
